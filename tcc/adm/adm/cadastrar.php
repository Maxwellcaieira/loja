...
if($btnCadUsuario){
	...
	if(in_array('',$dados)){
		...
		$result_usuario = "SELECT id FROM adm WHERE usuario='". $dados['usuario'] ."'";
		...
		$result_usuario = "SELECT id FROM adm WHERE email='". $dados['email'] ."'";
		$resultado_usuario = mysqli_query($conn, $result_usuario);
		if(($resultado_usuario) AND ($resultado_usuario->num_rows != 0)){
			$erro = true;
	...
	if(!$erro){
		...
		$result_usuario = "INSERT INTO adm (nome, email, usuario, senha) VALUES (
						'" .$dados['nome']. "',
						'" .$dados['email']. "',
						'" .$dados['usuario']. "',
						'" .$dados['senha']. "'
		...
		if(mysqli_insert_id($conn)){
			$_SESSION['msgcad'] = "<div class='alert alert-success'>Administrador cadastrado com sucesso!</div>";
			header("Location: login.php");
		}else{
			$_SESSION['msg'] = "<div class='alert alert-danger'>Erro ao cadastrar o Administraodr!</div>";
		}
	}
		...
		<div class="container">
			<div class="form-signin" style="background: #42dea4;">
				...
				<form method="POST" action="">
					...
					<!--<label>Usuário</label>-->
					<input type="text" name="usuario" placeholder="Digite seu CPF" class="form-control"><br>
